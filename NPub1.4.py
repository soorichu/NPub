...
from core import NCastJson, NCastToEpub, NewsJson, ContentToEpub, NewsTags, NewsToEpub
...
class Ui_Dialog(object):
 ...
 #       self.tabWidget.setTabEnabled(0, False)
    ...
    def ok_click(self):
        ...
        if selected_tab == 1: #NCast
        ...
        elif selected_tab == 0: #News
            for news in NewsTags.NewsTag:
                if news['title']==self.news_subject['title']:
                    self.news_subject['get_url'] = news['get_url']
                    self.news_subject['body_tag'] = news['body_tag']
                    break
            ...
            if len(self.news_subject)>1:
                ...
                ns = NewsToEpub.NewsToEpub('news', self.news_subject, self.news_format, self.news_number)
                sys.exit(app.exec_())
            else:
            #    print("모두 선택해주세요.")
                pass
